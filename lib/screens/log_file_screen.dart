...
import '../viewmodels/log_file_view_model.dart';
import '../widgets/app_drawer.dart';
import '../widgets/common/error_display.dart';

/// A reusable log file screen that works for any OPNsense log endpoint.
///
/// Pass [title] for the AppBar, [currentRoute] for the active drawer highlight,
/// [fetcher] for the API call, and [defaultSeverities] to pre-select the
/// appropriate severity levels for that log source.
///
/// Used by OpenVPN, WireGuard, Audit, Backend, Boot, General, and Web GUI log
/// screens.
class LogFileScreen extends StatefulWidget {
  final String title;
  final String currentRoute;
  final LogFetcher fetcher;
  final List<String> defaultSeverities;
...
class _LogFileScreenState extends State<LogFileScreen> with SingleInitMixin {
  static const List<int> _rowCountOptions = <int>[50, 100, 200];

  late List<String> _severityOptions;
  final Set<String> _selectedSeverities = <String>{};
  final Set<int> _selectedLogIndexes = <int>{};

  late LogFileViewModel _viewModel;
  ...
  void onFirstDependency() {
    ...
    // Map default severity API strings to localised labels for pre-selection.
    final severityMap = <String, String Function()>{
      ...
      'Notice': () => l10n.notice,
      'Informational': () => l10n.info,
      'Debug': () => l10n.debug,
    };
    for (final sev in widget.defaultSeverities) {
      final label = severityMap[sev]?.call();
      if (label != null) _selectedSeverities.add(label);
    }

    _viewModel = LogFileViewModel(
      widget.fetcher,
      rowCount: _rowCount,
  ...
  Future<void> _onRefresh() async => _loadLogs(isRefresh: true);
  ...
  String _buildLogDetails(OpenvpnLogEntry log) {
    final details = StringBuffer()
      ..writeln('Timestamp: ${_formatTimestamp(log.timestamp)}')
      ..writeln('Severity: ${log.severity}')
      ..writeln('Process: ${log.processName ?? widget.title}');
        ...
        return Scaffold(
          ...
          body: Column(
            children: [
              ...
              Expanded(
                child: _buildBody(context, isLoading, errorMessage, logs),
              ),
            ],
          ),
        );
  ...
  Widget _buildFilters(bool isLoading) {
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text(
          '${_selectedSeverities.length} severities • $_selectedTimeFilter',
        ),
            ...
            child: Wrap(
              ...
              children: _severityOptions.map((severity) {
                ...
                return FilterChip(
                  ...
                  onSelected: (selected) =>
                      _toggleSeverity(severity, selected),
                );
              }).toList(),
            ),
          ),
  ...
  Future<void> _showLogDetails(OpenvpnLogEntry log) async {
      ...
      child: Row(
        children: [
          ...
          Padding(
            ...
            child: Text(l10n.pageOfTotal(_currentPage, _totalPages)),
          ),
          Expanded(
            child: OutlinedButton.icon(
